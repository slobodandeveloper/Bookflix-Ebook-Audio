...
public class FolioReader {
    ...
    public static ReadTotalPagesListener readTotalPagesListener;
    ...
    public static final String ACTION_TOTAL_NO_OF_PAGES = "com.folioreader.action.TOTAL_PAGES";
    public static final String EXTRA_TOTAL_NO_OF_PAGES = "com.folioreader.extra.TOTAL_PAGES";
    ...
    private BroadcastReceiver totalPageReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {

            String totalNoOfPages =
                    intent.getStringExtra(FolioReader.EXTRA_TOTAL_NO_OF_PAGES);
            if (readTotalPagesListener != null)
                readTotalPagesListener.saveTotalPages(totalNoOfPages);

        }
    };
    private boolean isNightMode = false;

    public static FolioReader getInstance(Context context, ReadTotalPagesListener readTotalPagesListener) {
        ...
        FolioReader.readTotalPagesListener = readTotalPagesListener;
    ...
    private FolioReader(Context context) {
        ...
        /*LocalBroadcastManager.getInstance(context).registerReceiver(totalPageReceiver,
                new IntentFilter(ACTION_TOTAL_NO_OF_PAGES));*/
    }


    public FolioReader saveThemeOption(boolean isNightMode) {
    ...
    private Intent getIntentFromUrl(String assetOrSdcardPath, int rawId) {
        ...
        if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.M)
            intent.setFlags(FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_MULTIPLE_TASK);
    ...
    public FolioReader setTotalPagesListener(ReadTotalPagesListener totalPagesListener) {
        this.readTotalPagesListener = totalPagesListener;
        return singleton;
    }
    ...
    private void unregisterListeners() {
        ...
        //LocalBroadcastManager.getInstance(context).unregisterReceiver(totalPageReceiver);
    }
}
